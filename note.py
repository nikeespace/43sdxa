...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS profiles
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, avatar TEXT, remark TEXT, account_number TEXT, link TEXT)''')
...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --addr-card-bg: rgba(0,0,0,0.2); /* 半透明，适应背景色 */
            --addr-text: rgba(255,255,255,0.7); /* 默认左侧文字颜色 */
            --btn-bg: #e0e0e0;
            --highlight: #3498db;
        ...
        * { box-sizing: border-box; }
        ...
        .profile-top { display: flex; gap: 8px; margin-bottom: 8px; align-items: center; }
        .profile-avatar {
        ...
        .profile-name { font-weight: bold; font-size: 0.95em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .profile-remark { font-size: 0.75em; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        .profile-bottom {
            border-radius: 4px; padding: 4px; display: flex; align-items: center; justify-content: space-between; font-size: 0.8em;
        }
        .acc-num { font-family: monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 80px; }
        ...
        .profile-actions { position: absolute; top: 2px; right: 2px; display: flex; gap: 2px; }
        ...
        #authPassword {
            width: 100%; margin: 20px 0; padding: 15px; border: 2px solid var(--highlight); border-radius: 8px; background: var(--input-bg); color: var(--text-color); 
            font-size: 24px; font-weight: bold; text-align: center; letter-spacing: 2px;
        }
        ...
        .color-picker-label { font-size: 0.9em; font-weight: bold; }
        ...
        @media (max-width: 768px) {
            ...
            .sidebar-right { width: 100%; position: relative; height: auto; display: block; padding-top: 20px; border-left: none; border-top: 1px solid var(--border-color);}
...
<body>
    ...
    <aside class="sidebar-left">
        ...
        <div class="sidebar-desc" style="margin-top: auto; opacity: 0.5; text-align: center;">LegendVPS Tool v4.2 (Color)</div>
    </aside>
    ...
    <div class="modal-overlay" id="authModal">
        <div class="modal-box">
            <h3>请验证</h3>
            <input type="password" id="authPassword" maxlength="10">
    ...
    <div class="modal-overlay" id="colorModal">
        <div class="modal-box">
            <h3>🎨 界面配色</h3>
            <form id="colorForm" action="/save_theme" method="post" onsubmit="event.preventDefault(); triggerAuth('save_theme', this);">
                <div class="color-picker-row">
                    <span class="color-picker-label">👈 左侧背景</span>
                    <input type="color" name="left_bg" value="{{ settings.get('left_bg', '#2c3e50') }}">
                </div>
                <div class="color-picker-row">
                    <span class="color-picker-label">👈 左侧文字</span>
                    <input type="color" name="left_text" value="{{ settings.get('left_text', '#ffffff') }}">
                </div>
                <hr style="border:0; border-top:1px dashed #ddd; margin:15px 0;">
                <div class="color-picker-row">
                    <span class="color-picker-label">👉 右侧背景</span>
                    <input type="color" name="right_bg" value="{{ settings.get('right_bg', '#ffffff') }}">
                </div>
                <div class="color-picker-row">
                    <span class="color-picker-label">👉 右侧文字</span>
                    <input type="color" name="right_text" value="{{ settings.get('right_text', '#333333') }}">
                </div>
                <div class="modal-buttons">
                    <button type="button" class="btn-cancel" onclick="closeModal('colorModal')">取消</button>
                    <button type="submit" class="btn-confirm">保存 (需密码)</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if(passInput){
                passInput.addEventListener("keypress", function(event) { if (event.key === "Enter") confirmAction(); });
            }
        });
...
def index():
    ...
    for row in c.fetchall():
        ...
        items.append({
            ...
            'total_count': total_count, 'enable_stats': enable_stats, 'is_complete': (len(done_accs) >= total_count and total_count > 0)
        })
    conn.close()
...
def save_theme():
    left_bg = request.form['left_bg']
    left_text = request.form['left_text']
    right_bg = request.form['right_bg']
    right_text = request.form['right_text']
    
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('left_bg', left_bg))
    c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('left_text', left_text))
    c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('right_bg', right_bg))
    c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('right_text', right_text))
    conn.commit()
    conn.close()
...
def add_profile():
    ...
    if file and allowed_file(file.filename):
        filename = secure_filename(file.filename); filename = str(int(time.time())) + "_" + filename; file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('INSERT INTO profiles (name, avatar, remark, account_number, link) VALUES (?, ?, ?, ?, ?)', (name, filename, remark, account_number, link)); conn.commit(); conn.close()
    return redirect(url_for('index'))

@app.route('/edit_profile', methods=['POST'])
def edit_profile():
    pid = request.form['id']; name = request.form['name']; remark = request.form['remark']; account_number = request.form['account_number']; link = request.form['link']
    conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('UPDATE profiles SET name=?, remark=?, account_number=?, link=? WHERE id=?', (name, remark, account_number, link, pid)); conn.commit(); conn.close()
    return redirect(url_for('index'))
...
def edit_task_info(): task_id = request.form['id']; url = request.form['url']; remark = request.form['remark']; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('UPDATE bookmarks SET url = ?, remark = ? WHERE id = ?', (url, remark, task_id)); conn.commit(); conn.close(); return redirect(url_for('index'))

@app.route('/update_global_settings', methods=['POST'])
def update_global_settings(): raw_str = request.form['global_accounts_str']; new_list = [x.strip() for x in raw_str.replace('，', ',').split(',') if x.strip()]; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('global_accounts', json.dumps(new_list))); conn.commit(); conn.close(); return redirect(url_for('index'))

@app.route('/add_addr', methods=['POST'])
def add_addr(): name = request.form['name']; addr = request.form['addr']; uid = request.form['uid']; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('SELECT value FROM settings WHERE key = ?', ('address_book',)); row = c.fetchone(); current_list = json.loads(row[0]) if row else []; current_list.append({'name': name, 'addr': addr, 'uid': uid}); c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('address_book', json.dumps(current_list))); conn.commit(); conn.close(); return redirect(url_for('index'))

@app.route('/edit_addr', methods=['POST'])
def edit_addr(): index = int(request.form['index']); name = request.form['name']; addr = request.form['addr']; uid = request.form['uid']; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('SELECT value FROM settings WHERE key = ?', ('address_book',)); row = c.fetchone(); 
    if row: current_list = json.loads(row[0]); 
    if 0 <= index < len(current_list): current_list[index] = {'name': name, 'addr': addr, 'uid': uid}; c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('address_book', json.dumps(current_list))); conn.commit(); 
...
def delete_addr(index): conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('SELECT value FROM settings WHERE key = ?', ('address_book',)); row = c.fetchone(); 
    if row: current_list = json.loads(row[0]); 
    if 0 <= index < len(current_list): current_list.pop(index); c.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('address_book', json.dumps(current_list))); conn.commit(); 
    conn.close(); return redirect(url_for('index'))

@app.route('/add', methods=['POST'])
def add_entry(): url = request.form['url']; remark = request.form['remark']; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('SELECT value FROM settings WHERE key = ?', ('global_accounts',)); default_accs = json.loads(c.fetchone()[0]); c.execute('INSERT INTO bookmarks (url, remark, target_accounts, done_accounts, enable_stats) VALUES (?, ?, ?, ?, 1)', (url, remark, json.dumps(default_accs), '[]')); conn.commit(); conn.close(); return redirect(url_for('index'))
...
def update_item_accounts(id): raw_str = request.form['target_accounts_str']; new_list = [x.strip() for x in raw_str.replace('，', ',').split(',') if x.strip()]; conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('UPDATE bookmarks SET target_accounts = ? WHERE id = ?', (json.dumps(new_list), id)); conn.commit(); conn.close(); return redirect(url_for('index'))

@app.route('/update_progress/<int:id>', methods=['POST'])
def update_progress(id): done_accounts = request.form.getlist('done_accounts'); conn = sqlite3.connect(DB_FILE); c = conn.cursor(); c.execute('UPDATE bookmarks SET done_accounts = ? WHERE id = ?', (json.dumps(done_accounts), id)); conn.commit(); conn.close(); return redirect(url_for('index'))
